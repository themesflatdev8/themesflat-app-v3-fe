...
import Preview from "@/features/countdown-timer-bar-on-cart/components/Preview";
import Settings from "@/features/countdown-timer-bar-on-cart/components/Settings";
import General from "@/features/countdown-timer-bar-on-cart/components/General";
...
import { _settingsDefault } from "@/features/countdown-timer-bar-on-cart/constants";
...
const Customization = ({}: Props) => {
  ...
  const getForm = (form: IFormInput) => {
    return {
      ...
      timerSession: form?.timerSession,
      text: form?.text,
  ...
  useQuery(
    [_queryKey.getCustomization, { type: FEATURE_TYPE_COUNTDOWN_TIMER_BAR_ON_CART }],
    async () => {
      let res = await api.getSettings({ type: FEATURE_TYPE_COUNTDOWN_TIMER_BAR_ON_CART });
  ...
  let {
    ...
    [_queryKey.verifyAppBlock, { type: FEATURE_TYPE_COUNTDOWN_TIMER_BAR_ON_CART }],
    async () => {
      let res = await verifyAppApi.verifyAppBlock({ type: FEATURE_TYPE_COUNTDOWN_TIMER_BAR_ON_CART });
  ...
  const { mutate: mutateUpdate, isLoading: isLoadingSave } = useMutation(
    async (form: any) => {
      let res = await api.updateSettings({form, type: FEATURE_TYPE_COUNTDOWN_TIMER_BAR_ON_CART});
      let {
        status = false,
        data: dataRes = null,
        message = "",
