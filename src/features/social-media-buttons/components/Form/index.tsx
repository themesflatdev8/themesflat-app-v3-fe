...
import Preview from "@/features/social-media-buttons/components/Preview";
import Settings from "@/features/social-media-buttons/components/Settings";
import General from "@/features/social-media-buttons/components/General";
...
import { _settingsDefault } from "@/features/social-media-buttons/constants";
...
const Customization = ({}: Props) => {
  ...
  const getForm = (form: IFormInput) => {
    return {
      ...
      mobile: {
        visibility: form?.mobile?.visibility,
        template: form?.mobile?.template,
        size: form?.mobile?.size,
        position: form?.mobile?.position,
        positionBottom: form?.mobile?.positionBottom,
        positionLeft: form?.mobile?.positionLeft,
        positionRight: form?.mobile?.positionRight,
        style: form?.mobile?.style,
      },
      desktop: {
        visibility: form?.desktop?.visibility,
        template: form?.desktop?.template,
        size: form?.desktop?.size,
        position: form?.desktop?.position,
        positionBottom: form?.desktop?.positionBottom,
        positionLeft: form?.desktop?.positionLeft,
        positionRight: form?.desktop?.positionRight,
        style: form?.desktop?.style,
      }
    };
  };
  ...
  useQuery(
    [_queryKey.getCustomization, { type: FEATURE_TYPE_SOCIAL_MEDIA_BUTTONS }],
    async () => {
      let res = await api.getSettings({ type: FEATURE_TYPE_SOCIAL_MEDIA_BUTTONS });
  ...
  let {
    ...
    [_queryKey.verifyAppBlock, { type: FEATURE_TYPE_SOCIAL_MEDIA_BUTTONS }],
    async () => {
      let res = await verifyAppApi.verifyAppBlock({ type: FEATURE_TYPE_SOCIAL_MEDIA_BUTTONS });
  ...
  const { mutate: mutateUpdate, isLoading: isLoadingSave } = useMutation(
    async (form: any) => {
      let res = await api.updateSettings({form, type: FEATURE_TYPE_SOCIAL_MEDIA_BUTTONS});
      let {
        status = false,
        data: dataRes = null,
        message = "",
