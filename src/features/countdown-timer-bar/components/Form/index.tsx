...
import Preview from "@/features/countdown-timer-bar/components/Preview";
import Settings from "@/features/countdown-timer-bar/components/Settings";
import General from "@/features/countdown-timer-bar/components/General";
...
import { _settingsDefault } from "@/features/countdown-timer-bar/constants";
...
const Customization = ({}: Props) => {
  ...
  useQuery(
    [_queryKey.getCustomization, { type: FEATURE_TYPE_COUNTDOWN_TIMER_BAR }],
    async () => {
      let res = await api.getSettings({ type: FEATURE_TYPE_COUNTDOWN_TIMER_BAR });
  ...
  let {
    ...
    [_queryKey.verifyAppBlock, { type: FEATURE_TYPE_COUNTDOWN_TIMER_BAR }],
    async () => {
      let res = await verifyAppApi.verifyAppBlock({ type: FEATURE_TYPE_COUNTDOWN_TIMER_BAR });
  ...
  const { mutate: mutateUpdate, isLoading: isLoadingSave } = useMutation(
    async (form: any) => {
      let res = await api.updateSettings({form, type: FEATURE_TYPE_COUNTDOWN_TIMER_BAR});
      let {
        status = false,
        data: dataRes = null,
        message = "",
