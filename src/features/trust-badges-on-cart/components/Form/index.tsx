...
import Preview from "@/features/trust-badges-on-cart/components/Preview";
import Settings from "@/features/trust-badges-on-cart/components/Settings";
import General from "@/features/trust-badges-on-cart/components/General";
...
import { _settingsDefault } from "@/features/trust-badges-on-cart/constants";
...
const Customization = ({}: Props) => {
  ...
  useQuery(
    [_queryKey.getCustomization, { type: FEATURE_TYPE_TRUST_BADGES_ON_CART }],
    async () => {
      let res = await api.getSettings({ type: FEATURE_TYPE_TRUST_BADGES_ON_CART });
  ...
  let {
    ...
    [_queryKey.verifyAppBlock, { type: FEATURE_TYPE_TRUST_BADGES_ON_CART }],
    async () => {
      let res = await verifyAppApi.verifyAppBlock({ type: FEATURE_TYPE_TRUST_BADGES_ON_CART });
  ...
  const { mutate: mutateUpdate, isLoading: isLoadingSave } = useMutation(
    async (form: any) => {
      let res = await api.updateSettings({form, type: FEATURE_TYPE_TRUST_BADGES_ON_CART});
      let {
        status = false,
        data: dataRes = null,
        message = "",
