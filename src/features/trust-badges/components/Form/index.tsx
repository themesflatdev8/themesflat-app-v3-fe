...
import Preview from "@/features/trust-badges/components/Preview";
import Settings from "@/features/trust-badges/components/Settings";
import General from "@/features/trust-badges/components/General";
...
import { _settingsDefault } from "@/features/trust-badges/constants";
...
const Customization = ({}: Props) => {
  ...
  useQuery(
    [_queryKey.getCustomization, { type: FEATURE_TYPE_TRUST_BADGES }],
    async () => {
      let res = await api.getSettings({ type: FEATURE_TYPE_TRUST_BADGES });
  ...
  let {
    ...
    [_queryKey.verifyAppBlock, { type: FEATURE_TYPE_TRUST_BADGES }],
    async () => {
      let res = await verifyAppApi.verifyAppBlock({ type: FEATURE_TYPE_TRUST_BADGES });
  ...
  const { mutate: mutateUpdate, isLoading: isLoadingSave } = useMutation(
    async (form: any) => {
      let res = await api.updateSettings({form, type: FEATURE_TYPE_TRUST_BADGES});
      let {
        status = false,
        message = "",
      } = res;
