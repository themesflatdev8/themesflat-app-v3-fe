...
import Preview from "@/features/payment-badges/components/Preview";
import Settings from "@/features/payment-badges/components/Settings";
import General from "@/features/payment-badges/components/General";
...
import { _settingsDefault } from "@/features/payment-badges/constants";
...
const Customization = ({}: Props) => {
  ...
  useQuery(
    [_queryKey.getCustomization, { type: FEATURE_TYPE_PAYMENT_BADGES }],
    async () => {
      let res = await api.getSettings({ type: FEATURE_TYPE_PAYMENT_BADGES });
  ...
  let {
    ...
    [_queryKey.verifyAppBlock, { type: FEATURE_TYPE_PAYMENT_BADGES }],
    async () => {
      let res = await verifyAppApi.verifyAppBlock({ type: FEATURE_TYPE_PAYMENT_BADGES });
  ...
  const { mutate: mutateUpdate, isLoading: isLoadingSave } = useMutation(
    async (form: any) => {
      let res = await api.updateSettings({form, type: FEATURE_TYPE_PAYMENT_BADGES});
      let {
        status = false,
        message = "",
      } = res;
