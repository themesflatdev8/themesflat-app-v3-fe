...
import Preview from "@/features/sales-popup/components/Preview";
import Settings from "@/features/sales-popup/components/Settings";
import General from "@/features/sales-popup/components/General";
...
import { _settingsDefault } from "@/features/sales-popup/constants";
...
const Customization = ({}: Props) => {
  ...
  const getForm = (form: IFormInput) => {
    return {
      ...
      lastOrders: form?.lastOrders,
      statusDisplayOrder: form?.statusDisplayOrder,
      text: form?.text,
      placement: form?.placement,
      specificPages: form?.specificPages || [],
      timingFirst: form?.timingFirst,
      timingDelay: form?.timingDelay,
      timingDuration: form?.timingDuration,
      ...
      desktop: {
        ...
        positionTop: form?.desktop?.positionTop,
  ...
  useQuery(
    [_queryKey.getCustomization, { type: FEATURE_TYPE_SALES_POPUP }],
    async () => {
      let res = await api.getSettings({ type: FEATURE_TYPE_SALES_POPUP });
  ...
  let {
    ...
    [_queryKey.verifyAppBlock, { type: FEATURE_TYPE_SALES_POPUP }],
    async () => {
      let res = await verifyAppApi.verifyAppBlock({ type: FEATURE_TYPE_SALES_POPUP });
  ...
  const { mutate: mutateUpdate, isLoading: isLoadingSave } = useMutation(
    async (form: any) => {
      let res = await api.updateSettings({form, type: FEATURE_TYPE_SALES_POPUP});
      let {
        status = false,
        data: dataRes = null,
        message = "",
