...
import Preview from "@/features/payment-badges-on-cart/components/Preview";
import Settings from "@/features/payment-badges-on-cart/components/Settings";
import General from "@/features/payment-badges-on-cart/components/General";
...
import { _settingsDefault } from "@/features/payment-badges-on-cart/constants";
...
const schema = yup
  ...
  .shape({
    ...
    icons: yup.array().required(`This field is required.`),
...
const Customization = ({}: Props) => {
  ...
  const getForm = (form: IFormInput) => {
    return {
      ...
      strategySpecific: form.strategySpecific,
  ...
  useQuery(
    [_queryKey.getCustomization, { type: FEATURE_TYPE_PAYMENT_BADGES_ON_CART }],
    async () => {
      let res = await api.getSettings({ type: FEATURE_TYPE_PAYMENT_BADGES_ON_CART });
      ...
      if (status) {
        ...
        setFormDefault(() =>
          res.default ? clone(res.default) : clone(_settingsDefault)
        );
        return data;
      }
      return res;
  ...
  let {
    ...
    [_queryKey.verifyAppBlock, { type: FEATURE_TYPE_PAYMENT_BADGES_ON_CART }],
    async () => {
      let res = await verifyAppApi.verifyAppBlock({ type: FEATURE_TYPE_PAYMENT_BADGES_ON_CART });
  ...
  const { mutate: mutateUpdate, isLoading: isLoadingSave } = useMutation(
    async (form: any) => {
      let res = await api.updateSettings({form, type: FEATURE_TYPE_PAYMENT_BADGES_ON_CART});
      let {
        status = false,
        data: dataRes = null,
        message = "",
